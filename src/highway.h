...
public:
	...
	void saveNISOutput(std::string runDirectory) {
		// test if the run folder exist otherwise create it
		struct stat info;
		if(stat(runDirectory.c_str(), &info ) != 0 ){
			std::cout <<  "path does not exist (creating) :" << runDirectory << std::endl;
			// create run directory
			auto status = mkdir(runDirectory.c_str(), 0777);
			if (!status) {
					saveNISOutput(runDirectory);
				} else {
			...
			if( info.st_mode & S_IFDIR ) {
				std::cout << "(directory exist) -> saving NIS output for the runs" << std::endl;
				for (auto car: traffic) {
					tools.saveNIS(car, runDirectory);
				}
			}
			else {std::cerr << " path exists (and is not a directory) !" << std::endl;}
			}
	}
};